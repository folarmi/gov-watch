...
const Notifications = () => {
  ...
  const { data: notificationsData, isLoading } = useGetData({
    ...
    queryKey: ["GetNotifications", JSON.stringify(pageNumber)],
  });
  ...
  useEffect(() => {
    if (notificationsData) {
      if (pageNumber === 1) {
        setItems(notificationsData); // Reset items for the first page
      } else {
        setItems((prevItems) => [...prevItems, ...notificationsData]); // Append new data
      }

      // Check if there's more data to load
      if (
        notificationsData.length === 0 ||
        notificationsData.length < pageSize
      ) {
  ...
  console.log(userType === userTypeObject.admin);
  return (
    <DashboardLayout>
      {isLoading ? (
        <Loader />
